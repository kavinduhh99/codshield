...
export async function POST(req: Request) {
  try {
    ...
    const normalize = (p: string) => p.replace(/[^0-9+]/g, "").replace(/^0/, "+94");
    ...
    const finalOrders = orders.map((order: any) => {
      const normalizedPhone = order.phone ? normalize(order.phone) : undefined;
      const normalizedPhone2 = order.phone2 ? normalize(order.phone2) : undefined;
    ...
    }).filter((o: any) => o.phone && o.address && o.city);

    if (finalOrders.length === 0) {
      return NextResponse.json({ message: "No valid orders to insert" }, { status: 400 });
    }
